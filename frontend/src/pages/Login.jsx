...
export default function Login() {
  ...
  const t = {
    en: { welcome: "Welcome Back", emailLabel: "Email ID", pass: "Password", login: "Login", google: "Continue with Google", or: "OR", noAcc: "New traveler?", signup: "Create Account", tag: "Explore South India", err: "Invalid Credentials" },
    ta: { welcome: "நல்வரவு", emailLabel: "மின்னஞ்சல்", pass: "கடவுச்சொல்", login: "உள்நுழை", google: "கூகுள் மூலம் தொடரவும்", or: "அல்லது", noAcc: "புதிய பயணியா?", signup: "பதிவு செய்க", tag: "தென்னிந்தியாவை ஆராயுங்கள்", err: "தவறான விவரங்கள்" }
  }[lang];
  ...
  const handleGoogleLogin = async () => {
    ...
    // Forces the browser to show the account selection window
    ...
    try {
      ...
      setError(err.code === "auth/unauthorized-domain" ? "Domain not authorized in Firebase." : "Google Sign-In failed.");
    }
  };
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 relative overflow-y-auto">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass w-full max-w-[400px] p-8 md:p-10 z-10">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-black italic text-white drop-shadow-md">TN Flow</h1>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="auth-input-pill" />
          </div>

          <div className="relative">
            <label className="auth-label">{t.pass}</label>
            <input type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="auth-input-pill" />
          ...
          <button type="submit" className="btn-pill">{t.login}</button>
        </form>
